       ...
       // private readonly IMapper Mapper;
        ...
        public async void ScanBarcode(string code)
        {
            try
            {
                ...
                var labelEnityes = StockContract.StockDtos.Where(a => a.MaterialCode == SearchBarcode).FirstOrDefault();
                var materiales = MaterialContract.Materials.FirstOrDefault(a => a.Code == SearchBarcode);

                if (labelEnityes == null)
                {
                    Quantities = 0;
                }
                else
                {
                    Quantities = labelEnityes.Quantity;
                }
                if (labelEnityes == null || Quantities < materiales.MaxNum)
                {
